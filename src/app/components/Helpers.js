...
export function initCenteringGuidelines(canvas) {
  ...
  for (
    var j = canvasHeightCenter - centerLineMargin,
      lens = canvasHeightCenter + centerLineMargin;
    j <= lens;
    j++
  ) {
...
export function selectObject(canvas, activeObject) {
  $(".iconbar").show();
  if (canvas) {
    if (!activeObject) activeObject = canvas.getActiveObject();
