    ...
    this.canvas.on({
      ...
      "object:moving": (e) => {
        ...
        if (this.props.state.isSnap) {
          e.target.set({
            left:
              Math.round(e.target.left / this.props.state.gridsize) *
              this.props.state.gridsize,
            top:
              Math.round(e.target.top / this.props.state.gridsize) *
              this.props.state.gridsize
          });
        }
      },
      ...
      "selection:created": (e) => {
        ...
        } else selectObject(lthis.canvas);
      },
      "selection:updated": () => {
        lthis.updateState();
  ...
  avoidOverlap = (e) => {
    ...
    this.canvas.forEachObject(function (obj) {
      ...
      if (
        ...
        if (
          Math.abs(
            e.target.get("left") - (obj.get("left") + obj.get("width"))
          ) < snap
        ) {
          e.target.set({
        ...
        if (
          Math.abs(
            e.target.get("left") + e.target.get("width") - obj.get("left")
          ) < snap
        ) {
          e.target.set({
      ...
      if (
        ...
        if (
          Math.abs(e.target.get("top") - (obj.get("top") + obj.get("height"))) <
          snap
        ) {
          e.target.set({
        ...
        if (
          Math.abs(
            e.target.get("top") + e.target.get("height") - obj.get("top")
          ) < snap
        ) {
          e.target.set({
            left: obj.get("left") + obj.get("width") - e.target.get("width")
